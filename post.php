...
if (autenticar($db_con)) {
    ...
    if (isset($_POST['texto']) || isset($_FILES['imagem'])) {
        ...
        if (isset($_FILES['imagem'])) {
            ...
            // Verificação de tipo de arquivo (aceitando apenas imagens JPEG, PNG)
            ...
            if (in_array($file_type, $valid_types)) {
                ...
                $resposta["sucesso"] = 0;
                $resposta["erro"] = "Tipo de arquivo inválido. Apenas JPEG e PNG são permitidos.";
                $resposta["cod_erro"] = 2;
                echo json_encode($resposta);
                exit;
            }
        ...
        $consulta = $db_con->prepare("INSERT INTO post(nome, descricao, img, usuarios_login) 
                                      VALUES(:nome, :descricao, :img_url, :login)");

        // Vincula os parâmetros de forma segura
        $consulta->bindParam(':nome', $texto, PDO::PARAM_STR);
